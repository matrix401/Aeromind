      ...
      <div className="max-w-7xl mx-auto w-full">
        <div data-section-content>
          ...
          <h2 className="section-heading mb-4">
            Not reaction.
            <br />
            <span className="text-gradient-teal">Anticipation.</span>
          </h2>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {MOCK_ALERTS.map((alert) => (
              <div
                ...
                style={{ borderColor: `${LEVEL_COLOR[alert.level]}22`, borderWidth: 1 }}
              >
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <div className="font-mono text-sm font-medium text-[#F0F4F8]">{alert.flight}</div>
                    <div className="font-mono text-[10px] text-[#4A6278]">{alert.route}</div>
                  </div>
                  <div
                    className="font-mono text-[10px] px-2 py-1 rounded"
                    style={{
                      color: LEVEL_COLOR[alert.level],
                      background: `${LEVEL_COLOR[alert.level]}18`,
                    }}
                  >
                    {alert.level}
                  </div>
                ...
                <div className="mb-3">
                  <div className="flex justify-between mb-1">
                    <span className="font-mono text-[10px] text-[#4A6278]">Distress score</span>
                    <span className="font-mono text-[10px]" style={{ color: LEVEL_COLOR[alert.level] }}>
                      {alert.risk}/100
                    </span>
                  </div>
                ...
                <div className="space-y-1.5">
                  {alert.signals.map((sig) => (
                    <div key={sig} className="flex items-start gap-2">
                      ...
                      />
                      <span className="font-mono text-[10px] text-[#8BA3B8] leading-tight">{sig}</span>
                    </div>
                  ))}
                </div>
              </div>
